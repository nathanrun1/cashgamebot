...
USAGES = {
    ...
    "buyin": "lb buyin <player_name> <bank_player_name> <amount>"
}
...
async def debt(ctx, *args):
    ...
    try:
        ...
        if plr:
            ...
            relevant_debts = filter(lambda d: d.amount >= 0, all_debts)
            owed = {}
            for d in relevant_debts:
                recipient = get_user(user_id=d.recipient_id)
                if recipient:
                    plr = balances.get_player(recipient)
                    owed[f"{plr.name}"] += d.amount
            debt_table = t2a(
                header=["Owed To/By", "Amount"],
                body=list([k, owed[k]] for k in owed),
                style=PresetStyle.thin_compact
            )
            if owed:
...
@bot.command()
async def buyin(ctx, *args):
    if len(args) != 3:
        await ctx.send(f"Usage: ```{USAGES["buyin"]}```")
        return
    payer_name = args[0]
    bank_name = args[1]
    amount = args[2]
